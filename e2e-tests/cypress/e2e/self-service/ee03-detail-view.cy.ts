...
describe('Execution Environment Catalog and Detail View Tests', () => {
  beforeEach(() => {
    ...
    cy.get('body').then($body => {
      ...
      if (hasTableRows) {
        cy.log(`✅ Catalog has ${$body.find('table tbody tr').length} item(s) for testing`);
      } else {
        cy.log('ℹ️ Catalog is empty - some tests may be skipped');
        cy.log('ℹ️ Run ee02-template-execution.cy.ts first to create test data');
      }
    });
  });
    ...
    cy.get('body').then($body => {
      ...
      if (text.includes('Tags')) {
        ...
        cy.get('body').then($body => {
          ...
          const $tagsLabel = $body.find('*').filter((_, el) => {
            const $el = Cypress.$(el);
            const elText = $el.text().trim();
            return elText === 'Tags' || (elText.includes('Tags') && !$el.closest('table').length);
          }).first();

          if ($tagsLabel.length > 0) {
            // Try to find clickable element near Tags label
            // Check if Tags label itself is clickable
            if ($tagsLabel.is('button') || $tagsLabel.attr('role') === 'button' || $tagsLabel.is('input')) {
              ...
              const $clickable = $parent.find('button, [role="button"], input, select').first();
        ...
        cy.get('body').then($bodyAfter => {
          if ($bodyAfter.find('[role="listbox"], [role="menu"], [role="combobox"]').length > 0) {
      ...
      if (text.includes('Owner')) {
        ...
        cy.get('body').then($body => {
          ...
          const $ownerLabel = $body.find('*').filter((_, el) => {
            const $el = Cypress.$(el);
            const elText = $el.text().trim();
            return elText === 'Owner' || (elText.includes('Owner') && !$el.closest('table').length);
          }).first();

          if ($ownerLabel.length > 0) {
            // Try to find clickable element near Owner label
            if ($ownerLabel.is('button') || $ownerLabel.attr('role') === 'button' || $ownerLabel.is('input')) {
  ...
  it('Validates Catalog table: row elements (Name, Owner, Description, Tags, Actions)', () => {
    ...
    cy.get('body').then($body => {
      ...
      cy.log(`✅ Table found with ${$tableRows.length} row(s), checking row elements`);
      ...
      cy.get('table').first().find('tbody tr').first().then($firstRow => {
        ...
        // Check for Tags
        if ($firstRow.find('[class*="chip"], [class*="tag"], [class*="badge"]').length > 0) {
        ...
        const $starButton = $firstRow.find('button').filter((_, btn) => {
          ...
          return ariaLabel.includes('favorite') || ariaLabel.includes('star') ||
            title.includes('favorite') || title.includes('star') ||
        ...
        const $editButton = $firstRow.find('button').filter((_, btn) => {
          const ariaLabel = (btn.getAttribute('aria-label') || '').toLowerCase();
          const title = (btn.getAttribute('title') || '').toLowerCase();
          return ariaLabel.includes('edit') || title.includes('edit') ||
  ...
  it('Validates Catalog table: star/favorite button in Actions column', () => {
    ...
    cy.get('body').then($body => {
      ...
      // Wait a bit for any navigation/re-render to complete
      cy.wait(1000);
      ...
      cy.get('body').then($bodyAfter => {
        if ($bodyAfter.find('table tbody tr').length > 0) {
          ...
          cy.get('table tbody tr').first().within(() => {
            cy.get('button').then($buttonsAfter => {
              ...
              if ($starButtonAfter.length > 0) {
                ...
                cy.wait(1000);
                cy.log('✅ Clicked star/favorite button again - should remove from starred');
              } else {
          ...
          cy.log('ℹ️ Table not found after star click (may have navigated or re-rendered)');
        }
      });
    });
  });
  ...
  it('Validates Catalog table: clicking Name link navigates to detail view', () => {
    ...
    cy.get('body').then($body => {
      ...
      cy.get('table tbody tr').first().then($firstRow => {
        ...
        const $nameElement = $firstRow.find('a, button, [role="link"], [role="button"]')
          .filter((_, el) => {
        ...
        if ($nameElement.length > 0) {
          ...
          cy.url({ timeout: 20000 }).then(url => {
            if (url.includes('/self-service/catalog/') || url.includes('/catalog/')) {
              ...
              cy.get('body').then($detailBody => {
                ...
                if (hasMain) {
                  ...
                  cy.log('ℹ️ Main element not found (detail page may use different structure)');
                }
                ...
                if ($detailBody.text().includes(entityName)) {
                  ...
                  cy.log(`ℹ️ Entity name "${entityName}" not clearly visible on detail page`);
                }
                ...
                const hasDescription = $detailBody.text().includes('DESCRIPTION');

                if (hasOverview || hasAbout || hasDescription) {
                  cy.log('✅ Detail page content indicators found');
                } else {
                  cy.log('ℹ️ Detail page content indicators not clearly visible');
                }
              });
            } else {
          ...
          cy.get('table tbody tr').first().within(() => {
            cy.get('td').first().then($firstCell => {
              ...
              if (cellText) {
                ...
                cy.wrap($firstCell).find('*').first().click({ force: true });
                cy.wait(3000);
                ...
                cy.url().then(url => {
                  if (url.includes('/self-service/catalog/') || url.includes('/catalog/')) {
                    ...
                    cy.log(`ℹ️ After clicking first cell, URL is: ${url}`);
                  }
                });
              }
            });
          ...
          cy.url({ timeout: 20000 }).then(url => {
            if (url.includes('/self-service/catalog/') || url.includes('/catalog/')) {
              ...
              cy.get('body').then($detailBody => {
                ...
                if ($detailBody.text().includes('Links') || $detailBody.text().includes('Download')) {
                  ...
                  cy.contains('Download EE files').then(($downloadText: JQuery<HTMLElement>) => {
                    if ($downloadText && $downloadText.length > 0) {
                      // Find the parent Box that has onClick (clickable)
                      const $clickableBox = $downloadText.closest('div[style*="cursor: pointer"], div[style*="cursor:pointer"]')
                        .first();

                      if ($clickableBox.length > 0) {
                        cy.log('✅ Found Download EE files clickable element');
                ...
                if ($detailBody.text().includes('About')) {
                  ...
                  if ($detailBody.text().includes('OWNER')) {
                    ...
                    cy.contains('OWNER').then(($ownerLabel: JQuery<HTMLElement>) => {
                      if ($ownerLabel && $ownerLabel.length > 0) {
                        ...
                        const $ownerButton = $ownerLabel.parent().find('button').first();
                        ...
                        if ($ownerButton.length > 0) {
                          ...
                          cy.url().then(currentUrl => {
                            if (currentUrl !== url) {
                              cy.log(`✅ OWNER button navigated to: ${currentUrl}`);
                              cy.go('back');
                              cy.wait(2000);
                            } else {
                              cy.log('✅ OWNER button clicked (may have filtered or navigated)');
                            }
                          });
                        } else {
                ...
                cy.get('body').then($menuBody => {
                  ...
                  const $menuButton = $menuBody.find('button[aria-label], button')
                    .filter((_, btn) => {
                      // Check if button contains MoreVert icon (3 vertical dots)
                      const hasMoreVert = btn.querySelector('svg[data-testid*="MoreVert"], svg path[d*="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"]') !== null;
                      return hasMoreVert;
                    })
                    .first();
                  ...
                  if ($menuButton.length === 0) {
                    ...
                    cy.contains(entityName).then(($nameEl: JQuery<HTMLElement>) => {
                      if ($nameEl && $nameEl.length > 0) {
                        // Find button near the name (should be in header)
                        const $headerButtons = $nameEl.parent().parent().find('button').last();
                        if ($headerButtons.length > 0) {
                          cy.wrap($headerButtons).then($btn => {
                            cy.wrap($btn).click({ force: true });
                            cy.wait(1000);
                            cy.log('✅ Clicked menu button (fallback by position)');
                          });
                        }
                      }
                    });
                  ...
                  if ($menuButton.length > 0) {
                    ...
                    cy.get('body').then($menuOpenBody => {
                      ...
                      cy.contains('Inspect entity').then(($inspectOption: JQuery<HTMLElement>) => {
                        if ($inspectOption && $inspectOption.length > 0) {
                          ...
                          cy.get('body').then($inspectorBody => {
                            if ($inspectorBody.text().includes('Entity Inspector') ||
                              $inspectorBody.text().includes('Overview') ||
                              $inspectorBody.text().includes('Raw JSON') ||
                              $inspectorBody.text().includes('Raw YAML')) {
                              cy.log('✅ Entity Inspector opened successfully');

                              // Close Entity Inspector - look for Close button with defensive check
                              cy.get('body', { timeout: 5000 }).then($closeBody => {
                                // Try to find close button using multiple methods
                                const $closeButton = $closeBody.find('button, [role="button"]')
                                  .filter((_, btn) => {
                                    const text = (btn.textContent || '').toLowerCase().trim();
                                    const ariaLabel = (btn.getAttribute('aria-label') || '').toLowerCase();
                                    return text.includes('close') || ariaLabel.includes('close');
                                  })
                                  .first();
                                ...
                                if ($closeButton.length > 0) {
                                  ...
                                  cy.get('body').then($fallbackBody => {
                                    if ($fallbackBody.text().toLowerCase().includes('close')) {
                                      // Use cy.contains but with a timeout and error handling
                                      cy.contains('button', /close/i, { timeout: 3000 })
                                        .should('exist')
                                        .click({ force: true });
                                      cy.wait(2000);
                                      cy.log('✅ Closed Entity Inspector (fallback method)');
                                    } else {
                                      // Last resort: press Escape key
                                      cy.get('body').type('{esc}');
                                      cy.wait(1000);
                                      cy.log('✅ Closed Entity Inspector using Escape key');
                                    }
                                  });
                                }
                              });
                            } else {
                              cy.log('ℹ️ Entity Inspector may not have opened as expected');
                            }
                          });
                        } else {
                          cy.log('ℹ️ "Inspect entity" option not found in menu');
                        }
                      });
                    });
                    ...
                    cy.get('body').then($reopenBody => {
                      ...
                      const $menuButtonAgain = $reopenBody.find('button')
                        .filter((_, btn) => {
                          const hasMoreVert = btn.querySelector('svg[data-testid*="MoreVert"], svg path[d*="M12 8c1.1 0 2-.9 2-2"]') !== null;
                          return hasMoreVert;
                        })
                        .first();
                    ...
                    cy.get('body').then($unregisterBody => {
                      ...
                      cy.contains('Unregister entity').then(($unregisterOption: JQuery<HTMLElement>) => {                        if ($unregisterOption && $unregisterOption.length > 0) {
                          ...
                          cy.get('body').then($modalBody => {
                            if ($modalBody.text().includes('Are you sure') ||
                              $modalBody.text().includes('unregister') ||
                              $modalBody.text().includes('delete')) {
                              cy.log('✅ Unregister confirmation modal opened');
                              ...
                              cy.get('body').then($body => {
                                ...
                                const $deleteButton = $body.find('button, [role="button"]')
                                  .filter((_, btn) => {
                                    const text = (btn.textContent || '').toLowerCase().trim();
                                    const ariaLabel = (btn.getAttribute('aria-label') || '').toLowerCase();
                                    return (text.includes('delete') && text.includes('entity')) ||
                                      text === 'delete entity' ||
                                ...
                                if ($deleteButton.length > 0) {
                                  ...
                                  cy.log('✅ Found "Delete Entity" button directly (Flow 1)');
                                  ...
                                  cy.url({ timeout: 10000 }).then(currentUrl => {
                                    if (currentUrl.includes('/self-service/ee') ||
                                      currentUrl.includes('/catalog')) {
                                      cy.log(`✅ Navigated back to catalog after deletion: ${currentUrl}`);
                                    } else {
                                      cy.log(`ℹ️ Current URL after deletion: ${currentUrl}`);
                                    }
                                  });
                                } else {
                                  // Flow 2: Check for "Unregister entity" button in first modal
                                  cy.log('ℹ️ Delete Entity button not found - checking for Unregister entity button (Flow 2)');

                                  cy.get('body').then($flow2Body => {
                                    const $unregisterButton = $flow2Body.find('button, [role="button"]')
                                      .filter((_, btn) => {
                                        const text = (btn.textContent || '').toLowerCase().trim();
                                        const ariaLabel = (btn.getAttribute('aria-label') || '').toLowerCase();
                                        return (text.includes('unregister') && text.includes('entity')) ||
                                    ...
                                    if ($unregisterButton.length > 0) {
                                      ...
                                      cy.log('✅ Found "Unregister entity" button in first modal (Flow 2)');
                                      cy.wrap($unregisterButton).click({ force: true });
                                      cy.wait(2000);
                                      cy.log('✅ Clicked "Unregister entity" button in modal');

                                      // Now look for Delete Entity button in second modal
                                      cy.wait(1000);
                                      cy.get('body').then($secondModalBody => {
                                        const $deleteButton2 = $secondModalBody.find('button, [role="button"]')
                                          .filter((_, btn) => {
                                            const text = (btn.textContent || '').toLowerCase().trim();
                                            const ariaLabel = (btn.getAttribute('aria-label') || '').toLowerCase();
                                            return (text.includes('delete') && text.includes('entity')) ||
                                        ...
                                        if ($deleteButton2.length > 0) {
                                          cy.log('✅ Found "Delete Entity" button in second modal (Flow 2)');
                                          cy.wrap($deleteButton2).click({ force: true });
                                          cy.wait(2000);
                                          cy.log('✅ Clicked "Delete Entity" button');
                                          ...
                                          cy.log('ℹ️ Delete Entity button not found in second modal');
                                          // Try alternative selector
                                          cy.get('body').then($altBody => {
                                            if ($altBody.text().toLowerCase().includes('delete entity')) {
                                              cy.contains('button', 'Delete Entity', { timeout: 3000 })
                                                .should('exist')
                                                .click({ force: true });
                                              cy.wait(2000);
                                              cy.log('✅ Clicked "Delete Entity" button (alternative)');
                                            } else {
                                              // Close modal - use jQuery first to check existence
                                              cy.get('body').then($closeBody => {
                                                const $cancelBtn = $closeBody.find('button, [role="button"]')
                                                  .filter((_, btn) => {
                                                    const text = (btn.textContent || '').toLowerCase().trim();
                                                ...
                                                if ($cancelBtn.length > 0) {
                                                  ...
                                                  cy.get('body').type('{esc}');
                                                  cy.wait(1000);
                                                  cy.log('✅ Closed modal using Escape key');
                                                }
                                              });
                                            }
                                          });
                                      ...
                                      cy.log('ℹ️ Neither Delete Entity nor Unregister entity button found in modal');

                                      // Try to find Cancel button using jQuery first (defensive)
                                      cy.get('body').then($cancelBody => {
                                        const $cancelButton = $cancelBody.find('button, [role="button"]')
                                          .filter((_, btn) => {
                                            const text = (btn.textContent || '').toLowerCase().trim();
                                            return text.includes('cancel');
                                          })
                                        ...
                                        if ($cancelButton.length > 0) {
                                          ...
                                          cy.wait(1000);
                                          cy.log('✅ Clicked Cancel button to close modal');
                                        } else {
                                          // Last resort: press Escape key
                                          cy.get('body').type('{esc}');
                                          cy.wait(1000);
                              ...
                              cy.log('ℹ️ Unregister confirmation modal may not have opened');
                            }
                          });
                        } else {
                          cy.log('ℹ️ "Unregister entity" option not found in menu');
                        }
                      });
                    });
                  } else {
              ...
              cy.log(`ℹ️ Did not navigate to detail view. Current URL: ${url}`);
            }
          });
        } else {
          cy.log('ℹ️ Name element not found - cannot navigate to detail view');
        }
      });
    });
  });
