...
const REPO_SUFFIX = Math.floor(Math.random() * 100).toString().padStart(2, '0');
    ...
    it('Imports EE template via Add Template and executes it from Create tab', () => {
        ...
        cy.get('body').then($body => {
            if ($body.text().includes('Create')) {
                ...
                cy.log('ℹ️ Create tab not found on EE page - aborting template import test');
                return;
            }
        });
        ...
        cy.url({ timeout: 15000 }).then(url => {
            if (url.includes('/self-service/catalog-import')) {
                ...
                throw new Error('Catalog import page was not reached; aborting template execution test');
            }
        });


        cy.wait(3000);
        cy.url({ timeout: 15000 }).should('include', '/self-service/catalog-import');
        cy.get('main', { timeout: 15000 }).should('be.visible');
        ...
        cy.get('body').then($body => {
            ...
            if (!text.includes('review') && !text.includes('the following entities')) {
            ...
            if (importButton.length) {
                ...
                cy.log('ℹ️ Import button not found on Review step - wizard may behave differently');
            }
        });
        ...
        cy.get('body', { timeout: 20000 }).then($body => {
            ...
            if (finishText.toLowerCase().includes('the following entities have been added to the catalog')) {
            ...
            if ($body.text().includes(EE_TEMPLATE_TITLE)) {
                    ...
                    if ($body.text().includes('GitHub')) {
                        cy.contains(/github/i)
                            ...
                            .filter((_, el) => {
                                ...
                                return $el.hasClass('MuiCard-root') ||
                                    $el.is('button') ||
                                    $el.attr('role') === 'button' ||
                ...
                cy.get('body', { timeout: 30000 }).then($body => {
                    const hasCompletion =
                        ...
                        $body.find('[data-testid*="success"], [data-testid*="complete"]').length > 0;

                    if (!hasCompletion) {
                        cy.log('ℹ️ Completion page may still be loading, waiting additional time...');
                        cy.wait(5000);
                    }
                });
                ...
                cy.get('body', { timeout: 30000 }).then($body => {
                    const hasButtons =
                        ...
                        $body.find('button, a').filter((_, el) => {
                            ...
                            return text.includes('github') || text.includes('catalog') || 
                                   text.includes('getting started') || text.includes('download');
                        }).length > 0;
                ...
                //
                    ...
                    cy.get('body').then($body => {
                        if ($body.text().includes(EE_TEMPLATE_TITLE)) {
                            cy.log(`✅ Starting second execution of "${EE_TEMPLATE_TITLE}" template`);
                            ...
                            cy.get('body').then($bodyMCP => {
                                if ($bodyMCP.text().includes('GitHub')) {
                                    const $githubCard = $bodyMCP.find('*:contains("GitHub")')
                                        .filter((_, el) => {
                                            const $el = Cypress.$(el);
                                            const text = $el.text().toLowerCase();
                                            return text.includes('github') &&
                                                (text === 'github' || $el.parents('.MuiCard-root, [data-testid*="mcp"]').length > 0);
                                        })
                                        .closest('.MuiCard-root, [role="button"], button, div[onclick]')
                                        .first();
                            ...
                            cy.get('body').then($body => {
                                const $publishCheckbox = $body.find('input[type="checkbox"]')
                                    .filter((_, el) => {
                                        const $el = Cypress.$(el);
                                        const label = $el.closest('label, div').text().toLowerCase();
                                        return label.includes('publish') && label.includes('git') && label.includes('repository');
                                    })
                                    .first();
                                ...
                                if ($publishCheckbox.length > 0) {
                                    cy.wrap($publishCheckbox).then($checkbox => {
                                        if ($checkbox.is(':checked')) {
                                            ...
                                            cy.log('ℹ️ "Publish to a Git repository" checkbox already unchecked');
                                        }
                                    });
                                } else {
                                    ...
                                    cy.contains(/Publish to a Git repository/i)
                                        ...
                                        .uncheck({ force: true });
                                    cy.wait(500);
                                    cy.log('✅ Unchecked "Publish to a Git repository" checkbox (fallback method)');
                                }
                            });
                            ...
                            cy.url().then(finalUrl => {
                                cy.log(`✅ Completed second EE creation (without Git) wizard, final URL: ${finalUrl}`);
                            });
                            ...
                            cy.get('body', { timeout: 30000 }).then($body => {
                                ...
                                if (bodyText.includes('Download') || bodyText.includes('download') || bodyText.includes('EE files') || bodyText.includes('files')) {
                                    cy.log('✅ Found Download/EE files text on page');
                                    cy.get('body').then($b => {
                                        const $downloadBtn = $b.find('button, a, [role="button"]')
                                            .filter((_, el) => {
                                                const text = (el.textContent || '').toLowerCase();
                                                return (text.includes('download') && text.includes('ee')) ||
                                        ...
                                        if ($downloadBtn.length > 0) {
                                            ...
                                            cy.wrap($downloadBtn).then($btn => {
                                                ...
                                                if (href || downloadAttr) {
                                                    cy.log(`✅ EE files download button has href/download attribute: ${href || downloadAttr}`);
                                                }
                                    ...
                                    cy.log('ℹ️ EE files download button not found on completion page');
                                }

                                // 9b. Verify "Getting Started" button (second)
                                cy.get('body', { timeout: 15000 }).then($bodyAfter => {
                                    const bodyTextAfter = $bodyAfter.text();
                                    if (bodyTextAfter.includes('Getting Started') || bodyTextAfter.includes('Getting started') || bodyTextAfter.includes('getting started')) {
                                        cy.log('✅ Found Getting Started text on page');
                                        cy.url().then(originalUrl => {
                                            cy.get('body').then($b => {
                                                const $gettingStartedBtn = $b.find('button, a, [role="button"]')
                                                    .filter((_, el) => {
                                                        const text = (el.textContent || '').toLowerCase();
                                                        return text.includes('getting') && text.includes('started');
                                                    })
                                                    .first();
                                                ...
                                                if ($gettingStartedBtn.length > 0) {
                                                    ...
                                                    cy.wait(3000);
                                                    cy.url().then(url => {
                                                        if (url.includes('/getting-started') || url.includes('/docs') || url !== originalUrl) {
                                                            cy.log(`✅ Getting Started button navigated to: ${url}`);
                                                            cy.go('back');
                                                            cy.wait(2000);
                                                            cy.get('body', { timeout: 15000 }).should('be.visible');
                                                            cy.log('✅ Navigated back from Getting Started');
                                                        } else {
                                                            cy.log(`ℹ️ Getting Started button clicked, URL: ${url}`);
                                                        }
                                                    });
                                                } else {
                                        ...
                                        cy.log('ℹ️ Getting Started button not found on completion page');
                                    }
                                });
                                ...
                                cy.get('body', { timeout: 15000 }).then($bodyCatalog => {
                                    ...
                                    if (bodyTextCatalog.includes('View details in catalog') || bodyTextCatalog.includes('View details in Catalog') || bodyTextCatalog.includes('catalog')) {
                                        cy.log('✅ Found View details in Catalog text on page');
                                        cy.get('body').then($b => {
                                            const $catalogBtn = $b.find('button, a, [role="button"]')
                                                .filter((_, el) => {
                                                    const text = (el.textContent || '').toLowerCase();
                                                    return text.includes('view') && text.includes('catalog');
                                                })
                                                .first();
                                            ...
                                            if ($catalogBtn.length > 0) {
                                                ...
                                                cy.wait(2000);
                                                cy.log('✅ Clicked View details in Catalog button (opens in another window)');
                                                // Note: Since it opens in another window, we don't navigate back
                                            } else {
                                                cy.log('ℹ️ View details in Catalog button element not found');
                                            }
                                        });
                                    } else {
                                        cy.log('ℹ️ View details in Catalog button not found on completion page');
                                    }
                                });
                                ...
                                cy.get('body', { timeout: 15000 }).then($bodyGithub => {
                                    ...
                                    if (bodyTextGithub.includes('Github Repository') || bodyTextGithub.includes('GitHub Repository') || bodyTextGithub.includes('Github')) {
                                        cy.log('✅ Found Github Repository text on page');
                                        cy.get('body').then($b => {
                                            const $githubBtn = $b.find('button, a, [role="button"]')
                                                .filter((_, el) => {
                                                    const text = (el.textContent || '').toLowerCase();
                                                    return text.includes('github') && text.includes('repository');
                                                })
                                                .first();
                                            ...
                                            if ($githubBtn.length > 0) {
                                                cy.wrap($githubBtn).then($btn => {
                                                    ...
                                                    cy.url().then(originalUrl => {
                                                        if (href) {
                                                            cy.log(`✅ Github Repository link found with href: ${href}`);
                                                            ...
                                                            cy.url().then(url => {
                                                                if (url.includes('github.com')) {
                                                                    cy.log(`✅ Github Repository button navigated to GitHub: ${url}`);
                                                                    ...
                                                                    cy.log(`ℹ️ Github Repository button clicked, but URL is: ${url}`);
                                                            ...
                                                            cy.log('ℹ️ Github Repository button has no href, clicking as button');
                                                            cy.wrap($btn)
                                                                .invoke('removeAttr', 'target')
                                                                .click({ force: true });
                                                            cy.wait(3000);
                                                            cy.url().then(url => {
                                                                if (url.includes('github.com')) {
                                                                    ...
                                                                    cy.log(`ℹ️ Github Repository button clicked, URL: ${url}`);
                                                                }
                                                            });
                                                        }
                                                    });
                                                ...
                                                cy.log('ℹ️ Github Repository button element not found (text exists but no clickable element)');
                                            }
                                        });
                                    } else {
                                        cy.log('ℹ️ Github Repository button not found on completion page');
                                    }
                                });
                            });
                        } else {
