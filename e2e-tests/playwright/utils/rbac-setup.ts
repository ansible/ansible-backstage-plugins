...
async function createRBACRole(page: Page) {
  ...
  const roleName =
    process.env.RBAC_ROLE_NAME || DEFAULT_ROLE_NAME;
  ...
  const existingRole = page.getByText(roleName, { exact: true });
  ...
  for (const expandBtn of expandButtons) {
    ...
    // Re-query on each iteration to avoid stale handles after React re-renders
  ...
  // Verify the role was created by waiting for navigation back to roles list
  await page
...
export async function setupNonAdminRBAC(page: Page) {
