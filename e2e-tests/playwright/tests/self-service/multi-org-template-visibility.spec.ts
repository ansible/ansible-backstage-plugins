...
test('Template visibility: admin sees >= normal user templates', async ({
  ...
  const pageText = await normalPage.locator('body').innerText().catch(() => '');
  const got404 = pageText.includes("couldn't find that page") || pageText.includes('404');
  const gotForbidden = pageText.includes('Forbidden') || pageText.includes('not authorized');
  ...
  console.log(`[Visibility] Normal user sees ${normalCount} templates (404=${got404}, forbidden=${gotForbidden})`);
  ...
  expect(
    adminCount,
    'Admin should see at least 1 template',
  ).toBeGreaterThan(0);
  ...
  if (got404 || gotForbidden) {
    ...
    console.log(
      `[Visibility] Both users see the same ${adminCount} templates`,
    );
  }
});
