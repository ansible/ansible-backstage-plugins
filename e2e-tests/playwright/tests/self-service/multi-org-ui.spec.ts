...
test('Multi-Org UI: admin user entity page', async ({ page }) => {
  ...
  await expect(
    mainContent.getByText(/\[Default\]/i).first(),
  ).toBeVisible({ timeout: 10000 });
  await expect(
    mainContent.getByText(/\[11-org\]/i).first(),
  ).toBeVisible({ timeout: 10000 });
});
...
test('Multi-Org UI: catalog lists org group entities', async ({ page }) => {
  await page.goto(
    '/catalog?filters[kind]=group&filters[type]=organization',
    { waitUntil: 'domcontentloaded' },
  );
  await page.waitForLoadState('networkidle', { timeout: 30000 });
  ...
  await expect(
    tableOrList.getByText(/11org|11-org/i).first(),
  ).toBeVisible({ timeout: 15000 });
});
