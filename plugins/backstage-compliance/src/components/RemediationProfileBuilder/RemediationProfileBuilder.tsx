...
import {
  Typography, Button, Chip, Box, Collapse, IconButton,
  TextField, Divider, makeStyles, ButtonGroup, Tooltip,
...
import type { FindingSeverity, FindingState, MultiHostFinding, RemediationProfile } from '@ansible/backstage-compliance-common/types';
...
const useStyles = makeStyles(theme => ({
  ...
  summaryBar: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: theme.spacing(2), backgroundColor: theme.palette.background.default, borderRadius: theme.shape.borderRadius, marginBottom: theme.spacing(2), flexWrap: 'wrap' as const, gap: theme.spacing(1) },
  summaryCount: { display: 'flex', gap: theme.spacing(3), flexWrap: 'wrap' as const },
  bulkActions: { display: 'flex', gap: theme.spacing(1), flexWrap: 'wrap' as const, alignItems: 'center' },
  sectionHeader: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: theme.spacing(1.5, 2), backgroundColor: theme.palette.action.hover, borderRadius: theme.shape.borderRadius, marginBottom: theme.spacing(1), marginTop: theme.spacing(2), cursor: 'pointer' },
  ...
  passingNote: { display: 'flex', alignItems: 'center', gap: theme.spacing(1), padding: theme.spacing(1, 2), marginBottom: theme.spacing(1), color: theme.palette.text.secondary, fontSize: '0.85rem' },
  passingNoteIcon: { color: STATUS_COLORS.success, fontSize: 18 },
  filterBar: {
    display: 'flex', gap: theme.spacing(1), alignItems: 'center',
    flexWrap: 'wrap' as const, marginBottom: theme.spacing(1),
    [theme.breakpoints.down('md')]: { '& > :first-child': { flex: '1 1 100%' } },
  },
  selectionActions: {
    display: 'flex', gap: theme.spacing(1), flexWrap: 'wrap' as const, alignItems: 'center',
  },
}));
...
export const RemediationProfileBuilder = () => {
    ...
    api.getRemediationProfile(remediationId)
      .then(profile => {
        ...
        if (profile.complianceProfileId) {
          api.getRegisteredProfiles().then(rps => {
        ...
        return (scanIdToUse ? api.getFindings(scanIdToUse) : api.getFindings()).then(findings => ({ findings: findings ?? [], profile }));
      })
      .then(result => {
        if (!cancelled && result) {
          ...
          const stubs: MultiHostFinding[] = (prof.selections ?? [])
            ...
            .map(s => ({
              ruleId: s.ruleId, stigId: s.ruleId, title: s.ruleId, description: '', fixText: '', checkText: '',
              severity: 'CAT_II' as const, category: '', disruption: 'low' as const,
              aapImpact: 'safe' as const, aapImpactReason: '', parameters: [],
              hosts: [], passCount: 0, failCount: 0, naCount: 0, totalCount: 0,
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (profiles.length > 0) {
        const scan = scans.find(s => s.id === jobId || String(s.workflowJobId) === jobId);
        const matchedProfile = scan ? profiles.find(p => p.id === scan.profileId) : undefined;
  ...
  useEffect(() => {
    ...
    setSelections(prev => {
      ...
      allFindings.forEach(f => {
        ...
        initial[f.ruleId] = {
          enabled: saved ? saved.enabled : (isFailed ? (isEditMode ? false : f.disruption !== 'high' && f.aapImpact !== 'breaks-connectivity') : false),
          expanded: false,
  ...
  const toggleFinding = useCallback((ruleId: string) => { setSelections(prev => ({ ...prev, [ruleId]: { ...prev[ruleId], enabled: !prev[ruleId].enabled } })); }, []);
  const toggleExpanded = useCallback((ruleId: string) => { setSelections(prev => ({ ...prev, [ruleId]: { ...prev[ruleId], expanded: !prev[ruleId].expanded } })); }, []);
  const setScope = useCallback((ruleId: string, scope: RemediationScope) => { setSelections(prev => ({ ...prev, [ruleId]: { ...prev[ruleId], scope } })); }, []);
  const updateParameter = useCallback((ruleId: string, paramName: string, value: string | number | boolean) => { setSelections(prev => ({ ...prev, [ruleId]: { ...prev[ruleId], parameters: { ...prev[ruleId].parameters, [paramName]: value } } })); }, []);
  ...
  const handleRiskSelect = useCallback((key: string) => {
    if (key === 'all') { setDisruptionFilter('all'); setAapFilter('all'); return; }
    const [kind, value] = key.split(':');
    if (kind === 'disruption') { setAapFilter('all'); setDisruptionFilter(value); }
    else { setDisruptionFilter('all'); setAapFilter(value); }
  }, []);
  ...
  const activeFilterCount = [severityFilter, stateFilter, disruptionFilter, aapFilter].filter(f => f !== 'all').length;
  ...
  const displayFindings = useMemo(() => {
    ...
    if (stateFilter !== 'all') {
      result = result.filter(f => f.stateSummary && (f.stateSummary[stateFilter] ?? 0) > 0);
    }
    ...
    return result;
  }, [filteredFindings, allFindings, searchQuery, severityFilter, stateFilter, disruptionFilter, aapFilter, reviewMode, selections]);
  const failedFindings = useMemo(() => displayFindings.filter(f => f.failCount > 0), [displayFindings]);
  const passingFindings = useMemo(() => displayFindings.filter(f => f.failCount === 0), [displayFindings]);

  const selectAllFailed = () => { setSelections(prev => { const updated = { ...prev }; allFindings.forEach(f => { if (f.failCount > 0 && f.automationAvailable !== false) { updated[f.ruleId] = { ...updated[f.ruleId], enabled: true }; } }); return updated; }); };
  ...
  const selectDisplayed = () => { setSelections(prev => { const updated = { ...prev }; displayFindings.forEach(f => { if (f.automationAvailable !== false) { updated[f.ruleId] = { ...updated[f.ruleId], enabled: true }; } }); return updated; }); };
  const deselectDisplayed = () => { setSelections(prev => { const updated = { ...prev }; displayFindings.forEach(f => { updated[f.ruleId] = { ...updated[f.ruleId], enabled: false }; }); return updated; }); };
  const selectSafeOnly = () => {
    setSelections(prev => {
      const updated = { ...prev };
      allFindings.forEach(f => {
        if (f.failCount > 0 && f.aapImpact !== 'breaks-connectivity' && f.automationAvailable !== false) {
  ...
  const dominantScope = useMemo(() => {
    const scopes = allFindings.filter(f => f.failCount > 0).map(f => selections[f.ruleId]?.scope ?? 'failed_only');
    if (scopes.length === 0) return null;
    return scopes.every(s => s === 'standardize_all') ? 'standardize_all' : scopes.every(s => s === 'failed_only') ? 'failed_only' : null;
  ...
  const totalAffectedHosts = useMemo(() => { const hostSet = new Set<string>(); allFindings.forEach(f => { const sel = selections[f.ruleId]; if (sel?.enabled) { const failedHosts = f.hosts.filter(h => h.status === 'fail'); if (sel.scope === 'standardize_all' || failedHosts.length === 0) { f.hosts.forEach(h => hostSet.add(h.host)); } else { failedHosts.forEach(h => hostSet.add(h.host)); } } }); return hostSet.size; }, [allFindings, selections]);
  ...
  const getSeverityClass = useCallback((severity: FindingSeverity) => { switch (severity) { case 'CAT_I': return classes.catI; case 'CAT_II': return classes.catII; case 'CAT_III': return classes.catIII; default: return ''; } }, [classes.catI, classes.catII, classes.catIII]);
  ...
  const performAutoSave = useCallback(async () => {
    ...
    try {
      const allSelections = allFindings.map(f => ({ ruleId: f.ruleId, enabled: selectionsRef.current[f.ruleId]?.enabled ?? false, parameters: selectionsRef.current[f.ruleId]?.parameters ?? {}, scope: selectionsRef.current[f.ruleId]?.scope }));
      const effectiveScanId = isEditMode ? (editProfile?.creationScanId || editProfile?.complianceProfileId || '') : (jobId ?? '');
      const saved = await api.saveRemediationProfile({ id: savedProfileIdRef.current ?? (isEditMode ? editProfile?.id : undefined), name, description: profileDescriptionRef.current, complianceProfileId: editProfile?.complianceProfileId || resolvedProfileId, scanId: effectiveScanId, selections: allSelections, status: 'draft' });
      if (!savedProfileIdRef.current) { savedProfileIdRef.current = saved.id; setSavedProfileId(saved.id); }
      setAutoSaveStatus('saved');
    } catch (err) { setAutoSaveStatus('error'); setSaveError(err instanceof Error ? err.message : 'Auto-save failed'); } finally { savingInProgress.current = false; }
  }, [api, allFindings, editProfile, isEditMode, jobId]);

  useEffect(() => { if (profileSaved) setProfileSaved(false); }, [selections, profileName, profileDescription]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const renderAutoSaveStatus = () => {
    if (autoSaveStatus === 'saving') return (<div className={classes.autoSaveIndicator} data-testid="auto-save-status"><Typography variant="caption" style={{ color: STATUS_COLORS.neutral }}>Saving...</Typography></div>);
    if (autoSaveStatus === 'saved') return (<div className={classes.autoSaveIndicator} data-testid="auto-save-status"><CheckCircleIcon className={classes.savedIcon} /><Typography variant="caption" style={{ color: STATUS_COLORS.success }}>{profileSaved ? 'Saved' : 'Draft saved'}</Typography></div>);
    if (autoSaveStatus === 'error') return (<div className={classes.autoSaveIndicator} data-testid="auto-save-status"><WarningIcon style={{ fontSize: 14, color: STATUS_COLORS.error }} /><Typography variant="caption" style={{ color: STATUS_COLORS.error }}>Save failed</Typography></div>);
    return null;
  };
  ...
  const failedListItems = useMemo((): ListItem[] => {
    ...
    (['CAT_I', 'CAT_II', 'CAT_III'] as FindingSeverity[]).forEach(sev => {
      ...
      const sevEnabledCount = sevFindings.filter(f => selections[f.ruleId]?.enabled).length;
      items.push({ type: 'header', key: `failed-${sev}`, severity: sev, label: severityLabel[sev], count: sevFindings.length, enabledCount: sevEnabledCount });
  ...
  const categoryListItems = useMemo((): ListItem[] => {
    ...
    (['CAT_I', 'CAT_II', 'CAT_III'] as FindingSeverity[]).forEach(severity => {
      ...
      items.push({ type: 'header', key: `cat-${severity}`, severity, label: severityLabel[severity], count: group.length, enabledCount: groupEnabled });
  ...
  }), [selections, classes, toggleFinding, toggleExpanded, setScope, updateParameter, getSeverityClass]);

  const renderStatusView = () => {
    const failedGroupEnabled = failedFindings.filter(f => selections[f.ruleId]?.enabled).length;
    const passingGroupEnabled = passingFindings.filter(f => selections[f.ruleId]?.enabled).length;

    return (
      <>
        {failedFindings.length > 0 && (
          <>
            <div className={classes.sectionHeader} role="button" tabIndex={0} data-testid="failed-rules-header">
              <Box display="flex" alignItems="center" style={{ gap: 8 }}>
                <Chip label={`Rules with Failures (${failedFindings.length})`} size="small" style={{ backgroundColor: STATUS_COLORS.error, color: '#fff', fontWeight: 600 }} />
                <Typography variant="body2" color="textSecondary">{failedGroupEnabled}/{failedFindings.length} selected</Typography>
              </Box>
            </div>
            <VirtualizedFindingsList items={failedListItems} {...virtualListProps} />
          </>
        )}
        ...
        {passingFindings.length > 0 && (
          <>
            <div
              ...
              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setPassingRulesExpanded(!passingRulesExpanded); } }}
            >
              <Box display="flex" alignItems="center" style={{ gap: 8 }}>
                <Chip label={`Compliant Rules (${passingFindings.length})`} size="small" style={{ backgroundColor: STATUS_COLORS.success, color: '#fff', fontWeight: 600 }} />
                <Typography variant="body2" color="textSecondary">{passingGroupEnabled}/{passingFindings.length} selected</Typography>
              </Box>
              <IconButton size="small" aria-label={passingRulesExpanded ? 'Collapse compliant rules' : 'Expand compliant rules'}>
            ...
            <Collapse in={passingRulesExpanded}>
              <div className={classes.passingNote}>
                ...
                <Typography variant="body2" color="textSecondary">
                  These rules are currently compliant. Enable to enforce continuous compliance.
                </Typography>
              </div>
              <VirtualizedFindingsList items={passingListItems} listHeight={400} {...virtualListProps} />
            </Collapse>
          </>
        )}
      </>
  ...
  const renderCategoryView = () => {
    return <VirtualizedFindingsList items={categoryListItems} {...virtualListProps} />;
  };
  ...
  return (
    <>
      ...
      {(contextProfileName || contextInventoryName) && (
        <Box display="flex" alignItems="center" mt={1} mb={1} style={{ gap: 8 }}>
          {contextProfileName && (
            <Chip label={contextProfileName} size="small" variant="outlined" />
          )}
          {contextInventoryName && (
            <Chip label={contextInventoryName} size="small" variant="outlined" icon={<SettingsIcon style={{ fontSize: 14 }} />} />
          )}
        </Box>
      )}
      ...
      <div className={classes.filterBar}>
        <TextField
          ...
          InputProps={{
            startAdornment: <Box mr={1} display="flex" alignItems="center"><span role="img" aria-label="search">🔍</span></Box>,
          }}
        />
        <FilterGroup
          label="Severity"
        ...
        />
        <Chip label={`${displayFindings.length} rules`} variant="outlined" size="small" />
        {activeFilterCount > 0 && (
          <Chip label="Clear all" size="small" variant="outlined" onDelete={clearFilters} onClick={clearFilters} />
        )}
      </div>
      ...
      <div className={classes.summaryBar}>
        <div className={classes.summaryCount}>
          ...
          <Typography variant="body1" color="textSecondary"><strong>{passingFindings.length}</strong> compliant</Typography>
          <Typography variant="body1" style={{ color: STATUS_COLORS.success }}><strong>{enabledCount}</strong> selected</Typography>
        ...
        <div className={classes.bulkActions}>
          <ButtonGroup size="small" variant="outlined" className={classes.viewToggle} aria-label="View mode toggle">
            <Button onClick={() => setViewMode('status')} variant={viewMode === 'status' ? 'contained' : 'outlined'} color={viewMode === 'status' ? 'primary' : 'default'} data-testid="view-by-status">By Status</Button>
            <Button onClick={() => setViewMode('category')} variant={viewMode === 'category' ? 'contained' : 'outlined'} color={viewMode === 'category' ? 'primary' : 'default'} data-testid="view-by-category">By Category</Button>
          </ButtonGroup>
          ...
          {!isEditMode && (
            <ButtonGroup size="small" variant="outlined" aria-label="Host scope for all rules">
              <Tooltip title="Remediate only failed hosts for each rule">
                <Button onClick={() => setScopeAll('failed_only')} variant={dominantScope === 'failed_only' ? 'contained' : 'outlined'} color={dominantScope === 'failed_only' ? 'primary' : 'default'}>Failed Only</Button>
              </Tooltip>
              <Tooltip title="Apply to every host, standardizing configuration across the inventory">
                <Button onClick={() => setScopeAll('standardize_all')} variant={dominantScope === 'standardize_all' ? 'contained' : 'outlined'} color={dominantScope === 'standardize_all' ? 'primary' : 'default'}>All Hosts</Button>
              </Tooltip>
            </ButtonGroup>
          )}
        </div>
      ...
      <Box mb={2}>
        <div className={classes.selectionActions}>
          <Tooltip title="Enable all rules currently visible (respects search and filters)">
            <Button size="small" variant="outlined" onClick={selectDisplayed}>Select Displayed</Button>
          </Tooltip>
          <Tooltip title="Disable all rules currently visible">
            <Button size="small" variant="outlined" onClick={deselectDisplayed}>Deselect Displayed</Button>
          </Tooltip>
          <Tooltip title="Enable all rules that have at least one failing host">
            <Button size="small" variant="outlined" onClick={selectAllFailed}>Select All Failed</Button>
          </Tooltip>
          <Tooltip title="Enable all failed rules that are safe for AAP — excludes rules tagged 'Breaks AAP connectivity'">
            <Button size="small" variant="outlined" onClick={selectSafeOnly} style={{ borderColor: STATUS_COLORS.success, color: STATUS_COLORS.success }}>Select Safe Only</Button>
          </Tooltip>
          <Tooltip title="Disable every rule in this remediation">
            <Button size="small" variant="outlined" color="secondary" onClick={clearAll}>Deselect All</Button>
          </Tooltip>
        </div>
      </Box>
      ...
      <Box display="flex" justifyContent="flex-end" style={{ gap: 16 }}>
        <Button variant="outlined" color="primary" size="large" startIcon={<SaveIcon />} disabled={!profileName || enabledCount === 0 || profileSaved}
          onClick={async () => {
            if (savingInProgress.current) return;
            if (autoSaveTimerRef.current) { clearTimeout(autoSaveTimerRef.current); autoSaveTimerRef.current = null; }
            try {
              const allSelections = allFindings.map(f => ({ ruleId: f.ruleId, enabled: selections[f.ruleId]?.enabled ?? false, parameters: selections[f.ruleId]?.parameters ?? {}, scope: selections[f.ruleId]?.scope }));
              const effectiveScanId = isEditMode ? (editProfile?.creationScanId || editProfile?.complianceProfileId || '') : (jobId ?? '');
              const saved = await api.saveRemediationProfile({ id: savedProfileId ?? (isEditMode ? editProfile?.id : undefined), name: profileName, description: profileDescription, complianceProfileId: editProfile?.complianceProfileId || resolvedProfileId, scanId: effectiveScanId, selections: allSelections, status: 'saved' });
              if (!savedProfileId) { setSavedProfileId(saved.id); savedProfileIdRef.current = saved.id; }
        ...
        <Button variant="contained" color="primary" size="large" startIcon={<PlayArrowIcon />} disabled={enabledCount === 0 || !canRemediate || launching || !profileName} title={!canRemediate ? 'You do not have permission to apply remediations' : !profileName ? 'Enter a remediation name first' : undefined}
          onClick={async () => {
            setLaunching(true);
            try {
              const enabledSelections = allFindings.filter(f => selections[f.ruleId]?.enabled).map(f => ({ ruleId: f.ruleId, enabled: true, scope: selections[f.ruleId].scope ?? 'failed_only' as const, parameters: selections[f.ruleId].parameters }));
              const effectiveScanId = isEditMode ? (editProfile?.creationScanId || editProfile?.complianceProfileId || '') : (jobId ?? '');
              const saved = await api.saveRemediationProfile({ id: savedProfileId ?? (isEditMode ? editProfile?.id : undefined), name: profileName || editProfile?.name || 'Remediation', description: profileDescription || editProfile?.description || '', complianceProfileId: editProfile?.complianceProfileId || resolvedProfileId, scanId: effectiveScanId, selections: enabledSelections, status: 'saved' });
