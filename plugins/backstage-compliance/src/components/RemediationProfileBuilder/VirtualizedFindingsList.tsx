...
export const VirtualizedFindingsList: FC<VirtualizedFindingsListProps> = ({
  items,
  selections,
  onToggleFinding,
  onToggleExpanded,
  ...
  const itemsKey = useMemo(
    () =>
      items
        .map(it => (it.type === 'header' ? it.key : it.finding.ruleId))
        .join(','),
    [items],
  );
  ...
  useEffect(() => {
    ...
    items.forEach(item => {
      if (
        item.type === 'finding' &&
        selections[item.finding.ruleId]?.expanded
      ) {
  ...
  const measureRef = useCallback(
    (node: HTMLDivElement | null, ruleId: string) => {
      if (!node) {
  ...
  const effectiveHeight = useMemo(() => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.type === 'header') {
        ...
        totalHeight +=
          heightCache.current.get(item.finding.ruleId) ??
            ...
            <div className={classes.sectionHeader}>
              <Box display="flex" alignItems="center" style={{ gap: 8 }}>
                <Chip
                  label={item.label}
                  size="small"
                  className={`${severityChipClass} ${getSeverityClass(
                    item.severity,
                  )}`}
