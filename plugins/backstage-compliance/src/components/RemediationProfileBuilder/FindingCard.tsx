...
function arePropsEqual(
  prev: FindingCardProps,
  next: FindingCardProps,
): boolean {
    ...
    return (
      ...
      >
        <div className={classes.findingHeader}>
          <Switch
            checked={sel.enabled}
            onChange={() => onToggle(finding.ruleId)}
            color="primary"
            size="small"
            disabled={finding.automationAvailable === false}
            inputProps={{
              'aria-label': `Toggle rule ${finding.stigId} for remediation`,
            }}
          />
          ...
          <Chip
            label={(() => {
              if (finding.severity === 'CAT_I') return 'CAT I';
              if (finding.severity === 'CAT_II') return 'CAT II';
              return 'CAT III';
            })()}
            size="small"
          ...
          <div className={classes.titleGroup}>
            ...
            <div className={classes.hostBreakdown}>
              {finding.failCount > 0 ? (
                <>
                  <span style={{ color: STATUS_COLORS.error, fontWeight: 600 }}>
                    {finding.failCount} failed
                  </span>
                  <span>/</span>
                  <span>{finding.totalCount} hosts</span>
                  {finding.hosts.filter(h => h.status === 'fail').length >
                    0 && (
                ...
                <span style={{ color: STATUS_COLORS.success, fontWeight: 600 }}>
                  {finding.passCount}/{finding.totalCount} hosts passing
                </span>
              )}
            </div>
          </div>
            ...
            {finding.disruption && (
              <Box mb={2} display="flex" style={{ gap: 8 }}>
                <Chip
                  ...
                  style={{
                    backgroundColor: (() => {
                      if (finding.disruption === 'high')
                        return STATUS_COLORS.error;
                      if (finding.disruption === 'medium')
                ...
                />
                {finding.aapImpact === 'breaks-connectivity' && (
                  <Tooltip title={finding.aapImpactReason}>
                    <Chip
                      size="small"
            ...
            {finding.passCount > 0 && finding.failCount > 0 && (
              <div className={classes.adviceBanner}>
                ...
                <div>
                  ...
                  >
                    Only {finding.failCount} of {finding.totalCount} hosts
                    failed this rule.
                  </Typography>
                  <Typography variant="caption" style={{ color: '#856404' }}>
                    ...
                    {finding.hosts
                      ...
                      .map(h => h.host)
                      .join(', ')}{' '}
                    should belong to a different inventory with different
                ...
                if (finding.failCount === 0)
                  return (
                    <Typography
                      ...
                      style={{ marginTop: 4 }}
                    >
                      All {finding.totalCount} hosts are compliant — enabling
                      this rule enforces continuous compliance.
                    </Typography>
                  );
                if (finding.failCount === finding.totalCount)
                  return (
                    <Typography
                      variant="body2"
                      color="textSecondary"
                      style={{ marginTop: 4 }}
                    >
                      All {finding.totalCount} hosts failed — remediation will
                      apply to all hosts.
                    </Typography>
                  );
                return (
                  <RadioGroup
                    ...
                    onChange={e =>
                      onSetScope(
                        finding.ruleId,
                        e.target.value as RemediationScope,
                      )
                    }
                  >
                    <FormControlLabel
                      ...
                      label={
                        <Typography variant="body2">
                          Remediate failed hosts only ({finding.failCount}{' '}
                          hosts)
                        </Typography>
                      }
                    />
                    <FormControlLabel
                      ...
                      label={
                        <Typography variant="body2">
                          Apply to all hosts — standardize to same setting (
                          {finding.totalCount} hosts)
                        </Typography>
                      }
                    />
                  </RadioGroup>
                ...
                >
                  ...
                  {sel.scope === 'standardize_all' && (
                    <span style={{ fontStyle: 'italic' }}>
                      {' '}
                      — applied to all {finding.totalCount} hosts
                    </span>
                  )}
                </Typography>
                <Box
