  ...
  return (
    <g>
      {clusters.map((c: Cluster) => {
        ...
        const label = `${c.hosts.length} hosts · ${c.min.toFixed(
          0,
        )}–${c.max.toFixed(0)}%`;
        return (
          <g
          ...
          >
            <rect
              ...
              rx={3}
              fill={
                c.isOutlier ? 'rgba(201,25,11,0.08)' : 'rgba(62,134,53,0.08)'
              }
              stroke={color}
              strokeWidth={1}
              strokeOpacity={0.4}
...
export const StripPlot: FC<StripPlotProps> = ({
  hosts,
  allHosts,
  clusters,
  onClusterClick,
  ...
  const xMin = Math.max(
    0,
    Math.floor(Math.min(...hosts.map(h => h.compliancePct)) / 5) * 5 - 5,
  );
  const xMax = Math.min(
    100,
    Math.ceil(Math.max(...hosts.map(h => h.compliancePct)) / 5) * 5 + 5,
  );
  ...
  return (
    <div>
      <ResponsiveContainer width="100%" height={height || 180}>
        <ScatterChart margin={{ top: 24, right: 20, bottom: 20, left: 20 }}>
            ...
            content={({ payload }: any) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    {d.host.compliancePct}% · {d.host.failCount} failing
                  </div>
                  {d.host.catIFail > 0 && (
          ...
          {clusters.map(c => (
            <ReferenceArea
              key={c.id}
              x1={Math.max(xMin, c.min - 1.5)}
              x2={Math.min(xMax, c.max + 1.5)}
              y1={0}
              y2={1}
              fill={
                c.isOutlier ? 'rgba(201,25,11,0.06)' : 'rgba(62,134,53,0.04)'
              }
              stroke={c.isOutlier ? STATUS_COLORS.error : STATUS_COLORS.success}
              strokeDasharray="4 2"
              strokeOpacity={0.3}
            />
          ))}
      ...
      <div className={classes.hint}>
        ...
        {' · '}Click a dot for host details · Click a cluster label to zoom in ·{' '}
