...
export const ChipMatrix: FC<ChipMatrixProps> = ({
  hosts,
  allHosts,
  onHostClick,
  displayConfig,
  ...
  const outliers = hosts
    .filter(h => isOutlier(h, allHosts))
    .sort((a, b) => a.compliancePct - b.compliancePct);
  const nonCompliant = hosts
    .filter(h => !isOutlier(h, allHosts) && h.compliancePct < THRESHOLDS.good)
    .sort((a, b) => a.compliancePct - b.compliancePct);
  const yellow = hosts
    .filter(
      h =>
        !isOutlier(h, allHosts) &&
        h.compliancePct >= THRESHOLDS.good &&
        h.catIFail > 0,
    )
    .sort((a, b) => a.compliancePct - b.compliancePct);
  const green = hosts
        ...
        <Chip
          ...
          label={
            <div className={classes.chipLabel}>
              <span className={classes.hostname} style={{ color: style.color }}>
                {host.hostname}
              </span>
  ...
  return (
    <div>
      <Typography variant="body2" color="textSecondary" gutterBottom>
        {hosts.length} hosts · {outliers.length} outlier
        {outliers.length !== 1 ? 's' : ''}
