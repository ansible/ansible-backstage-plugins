...
export function useScanMetadata(
  ...
  const [scanType, setScanType] = useState<
    'assessment' | 'verification' | 'remediation'
  >('assessment');
  const [profileName, setProfileName] = useState('');
  const [frameworkLabel, setFrameworkLabel] = useState('');
  const [profileCert, setProfileCert] = useState<ScanCertification | null>(
    null,
  );
  const [profileDisplayConfig, setProfileDisplayConfig] = useState<
  ...
  const [resolvedWorkflowJobId, setResolvedWorkflowJobId] = useState<
    number | null
  >(null);
