  ...
  return (
    <InfoCard title={title || 'Verification Results'}>
        ...
        <div className={classes.comparisonGauge}>
          ...
          {beforeScanLabel && (
            <Typography
              ...
              style={onBeforeScanClick ? { cursor: 'pointer', textDecoration: 'underline' } : undefined}
          ...
          {beforeBaselineRate !== undefined && (
            <Typography variant="caption" style={{ display: 'block', marginTop: 2, fontWeight: 600, color: scoreColor(isBaselineView ? (beforeStandardRate ?? previousPassRate) : beforeBaselineRate) }}>
              {isBaselineView ? `Standard: ${beforeStandardRate ?? previousPassRate}%` : `Baseline: ${beforeBaselineRate}%`}
            </Typography>
          )}
        </div>
        ...
        <div className={classes.comparisonGauge}>
          ...
          {afterScanLabel && (
            <Typography
              ...
              style={onAfterScanClick ? { cursor: 'pointer', textDecoration: 'underline' } : undefined}
          ...
          {afterBaselineRate !== undefined && (
            <Typography variant="caption" style={{ display: 'block', marginTop: 2, fontWeight: 600, color: scoreColor(isBaselineView ? (afterStandardRate ?? overallPassRate) : afterBaselineRate) }}>
              {isBaselineView ? `Standard: ${afterStandardRate ?? overallPassRate}%` : `Baseline: ${afterBaselineRate}%`}
            </Typography>
          )}
        </div>
      </div>
      <div className={classes.deltaStats}>
        ...
        <Tooltip title="Click to filter: rules where all hosts changed from failing to passing" arrow>
          <div
            className={`${classes.deltaStat} ${onStatClick ? classes.clickableStat : ''}`}
            onClick={() => onStatClick?.(activeComparison === 'improved' ? 'all' : 'improved')}
            style={activeComparison === 'improved' ? { backgroundColor: 'rgba(62,134,53,0.1)' } : undefined}
          >
            <TrendingUpIcon fontSize="small" style={{ color: STATUS_COLORS.success }} />
        ...
        {comparisonStats.regressed > 0 && (
          <Tooltip title="Click to filter: rules that were passing but now have failures" arrow>
            <div
              className={`${classes.deltaStat} ${onStatClick ? classes.clickableStat : ''}`}
              onClick={() => onStatClick?.(activeComparison === 'regressed' ? 'all' : 'regressed')}
              style={activeComparison === 'regressed' ? { backgroundColor: 'rgba(201,25,11,0.1)' } : undefined}
            >
              <TrendingDownIcon fontSize="small" style={{ color: STATUS_COLORS.error }} />
        ...
        <Tooltip title="Click to filter: rules with no change between scans" arrow>
          <div
            className={`${classes.deltaStat} ${onStatClick ? classes.clickableStat : ''}`}
            onClick={() => onStatClick?.(activeComparison === 'unchanged' ? 'all' : 'unchanged')}
            style={activeComparison === 'unchanged' ? { backgroundColor: 'rgba(106,110,115,0.1)' } : undefined}
          >
            <RemoveIcon fontSize="small" style={{ color: STATUS_COLORS.neutral }} />
