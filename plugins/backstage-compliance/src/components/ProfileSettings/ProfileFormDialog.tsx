...
export const ProfileFormDialog = ({
  ...
  const [pendingAutoPopulate, setPendingAutoPopulate] = useState<Record<string, unknown> | null>(null);
  ...
  useEffect(() => {
    if (!autoPopulateEnabled || !form.workflowTemplateId) return;
    if (skipNextAutoPopulateRef.current) {
      skipNextAutoPopulateRef.current = false;
      return;
    }
    let cancelled = false;
    api.getJobTemplateDetail(form.workflowTemplateId).then(detail => {
      if (cancelled) return;
      try {
  ...
  const applyAutoPopulate = (vars: Record<string, unknown>) => {
    ...
    const displayConfig = vars.display_config as Record<string, unknown> | undefined;

    setForm(prev => ({
      ...prev,
      profileSlug: (vars.profile_slug as string) || prev.profileSlug || slugify((vars.profile_name as string) || prev.displayName),
      ...
      remediationPlaybookPath: (vars.remediation_playbook as string) || prev.remediationPlaybookPath,
      ...
      platformSpec: {
        ...
        os_version: (vars.os_version as string[]) || prev.platformSpec?.os_version,
      },
      ...(displayConfig && typeof displayConfig === 'object' ? { displayConfig: displayConfig as import('@ansible/backstage-compliance-common').ProfileDisplayConfig } : {}),
    }));
    if (vars.certification_status && ['certified', 'conformant', 'uncertified'].includes(vars.certification_status as string)) {
  ...
  const handleSave = async () => {
    ...
    setSaveError(null);
    try {
      const certification = { status: certStatus as CertificationStatus, authority: certAuthority, validationId: '', disclaimer: '' };
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        <DialogTitle>{editProfile ? 'Edit Compliance Profile' : 'Add Compliance Profile'}</DialogTitle>
        <DialogContent>
          <div className={classes.dialogContent}>
            ...
            <Typography variant="caption" color="textSecondary" style={{ marginBottom: 8 }}>
              Select a scan job template to auto-detect profile configuration from its default extra_vars.
            </Typography>
            ...
            <FormControl variant="outlined" fullWidth className={classes.formField}>
              ...
              >
                ...
                {executionEnvironments.map(ee => (
                  <MenuItem key={ee.id} value={ee.id}>{ee.name}</MenuItem>
                ))}
              </Select>
            </FormControl>
            ...
            <TextField label="Profile Name" variant="outlined" fullWidth required
              value={form.displayName} onChange={e => {
            ...
            <TextField label="Profile Slug" variant="outlined" fullWidth
              value={form.profileSlug ?? ''} onChange={e => updateForm('profileSlug', e.target.value)}
              helperText={editProfile ? 'Slug is immutable after creation' : 'Stable identifier for reconnect — auto-derived from name'}
            ...
            <Autocomplete
              ...
              renderInput={params => (
                <TextField {...params} label="Compliance Standard" variant="outlined" placeholder="e.g., DISA STIG, CIS, SOC2" className={classes.formField} />
              )}
            />

            <TextField label="Standard Version" variant="outlined" fullWidth placeholder="e.g., V2R8"
              value={form.version} onChange={e => updateForm('version', e.target.value)} className={classes.formField} />

            <TextField label="Target Platform" variant="outlined" fullWidth placeholder="e.g., RHEL 9"
              value={form.platform} onChange={e => updateForm('platform', e.target.value)} className={classes.formField} />

            <Typography variant="subtitle2" style={{ marginTop: 16, marginBottom: 4 }}>
              Platform Guards
            </Typography>

            <TextField label="OS Family" variant="outlined" fullWidth
              value={form.platformSpec?.os_family?.join(', ') || ''}
              onChange={e => {
                const val = e.target.value;
                setForm(prev => ({ ...prev, platformSpec: { ...prev.platformSpec, os_family: val ? val.split(',').map(s => s.trim()).filter(Boolean) : undefined } }));
              }}
            ...
            <TextField label="OS Version" variant="outlined" fullWidth
              ...
              onChange={e => {
                const val = e.target.value;
                setForm(prev => ({ ...prev, platformSpec: { ...prev.platformSpec, os_version: val ? val.split(',').map(s => s.trim()).filter(Boolean) : undefined } }));
              }}
              helperText="Comma-separated: 9, 8, 2022, etc."
              className={classes.formField} />

            <TextField label="Device Type" variant="outlined" fullWidth
              value={form.platformSpec?.device_type?.join(', ') || ''}
              onChange={e => {
                const val = e.target.value;
                setForm(prev => ({ ...prev, platformSpec: { ...prev.platformSpec, device_type: val ? val.split(',').map(s => s.trim()).filter(Boolean) : undefined } }));
              }}
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={e => {
                    setForm(prev => ({ ...prev, platformSpec: { ...prev.platformSpec, scanner_validates: e.target.checked || undefined } }));
                  }}
                />
              }
            ...
            <TextField select label="Certification Status" value={certStatus}
              onChange={e => setCertStatus(e.target.value)} fullWidth size="small" className={classes.formField}>
              <MenuItem value="uncertified">Custom</MenuItem>
              <MenuItem value="conformant">Conformant</MenuItem>
              <MenuItem value="certified">Certified</MenuItem>
            </TextField>
            {certStatus !== 'uncertified' && (
              <TextField label="Certification Authority" value={certAuthority}
                onChange={e => setCertAuthority(e.target.value)} fullWidth size="small"
                helperText="e.g. NIST SCAP 1.2, CIS" className={classes.formField} />
            )}

            <TextField label="Scan Tags" variant="outlined" fullWidth
              placeholder="e.g., sshd_set_idle_timeout, accounts_tmout"
              value={form.scanTags} onChange={e => updateForm('scanTags', e.target.value)}
        ...
        <DialogActions>
          ...
          <Button variant="contained" color="primary" onClick={handleSave}
            ...
            {saving ? 'Saving...' : editProfile ? 'Update' : 'Save'}
          </Button>
        </DialogActions>
      </Dialog>

      <Dialog open={pendingAutoPopulate !== null} onClose={() => setPendingAutoPopulate(null)} maxWidth="xs" fullWidth>
        <DialogTitle>Apply Profile Configuration?</DialogTitle>
        <DialogContent>
          <Typography variant="body2">
            Automatic configuration data was detected from the selected job template's default extra_vars.
            Applying will overwrite the current form values.
          </Typography>
          {pendingAutoPopulate && (
            <Box mt={2} p={1.5} bgcolor="#f5f5f5" borderRadius={4} style={{ fontFamily: 'monospace', fontSize: 12 }}>
        ...
        <DialogActions>
          <Button onClick={() => setPendingAutoPopulate(null)}>Keep Manual Values</Button>
          <Button variant="contained" color="primary"
            onClick={() => pendingAutoPopulate && applyAutoPopulate(pendingAutoPopulate)}>
            Apply Configuration
          </Button>
        </DialogActions>
      </Dialog>
