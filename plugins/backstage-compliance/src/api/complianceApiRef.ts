...
export interface ComplianceApi {
  ...
  ): Promise<import('@ansible/backstage-compliance-common').PaginatedFindings>;
  ...
  ): Promise<HostPostureResponse>;
  getHostFindings(
    inventoryId: number,
    hostname: string,
    profileId: string,
    limit?: number,
  ): Promise<HostFindingsResponse>;
  ...
  saveRemediationProfile(
    body: SaveRemediationProfileRequest,
  ): Promise<RemediationProfile>;
  deleteRemediationProfile(id: string): Promise<void>;
  updateRemediationProfileStatus(
    id: string,
    status: RemediationProfileStatus,
  ): Promise<RemediationProfile>;
  getRemediationExecutions(
    profileId: string,
    limit?: number,
  ): Promise<RemediationExecution[]>;
  getBatchScanStats(scanIds: string[]): Promise<
    Record<
      string,
      {
        pass: number;
  ...
  getAuthoritativeScan(
    profileId: string,
    inventoryId: number,
  ): Promise<AuthoritativeScanResponse | null>;
  validateScan(body: {
    profileId: string;
    inventoryId: number;
  }): Promise<PlatformValidationResult>;
  getRegisteredProfiles(opts?: {
  ...
  >;
  getJobTemplateDetail(id: number): Promise<{
    id: number;
    name: string;
    description: string;
    extra_vars: string;
  ...
  getControllerJobTemplates(): Promise<
  ...
  getBaselineScores(remediationProfileId: string): Promise<
    Array<{
      inventoryId: number;
      passRate: number;
      passCount: number;
  ...
  getChain(
    executionId: string,
  ): Promise<import('@ansible/backstage-compliance-common').ChainResponse>;
  getArtifacts(
    scanId: string,
  ): Promise<import('@ansible/backstage-compliance-common').ScanArtifact[]>;
  downloadArtifact(
    scanId: string,
    artifactKey: string,
    filename: string,
