...
describe('AAPLogoutHandler', () => {
  ...
  it('should strip trailing slash from AAP base URL', async () => {
    mockConfigApi.getOptionalString.mockReturnValue('https://aap.example.com/');
  ...
  it('should not double-patch signOut when mounted twice with same identityApi', async () => {
    const { unmount: unmount1 } = await renderComponent();
    const patchedSignOut = mockIdentityApi.signOut;

    const { unmount: unmount2 } = await renderComponent();
    expect(mockIdentityApi.signOut).toBe(patchedSignOut);
