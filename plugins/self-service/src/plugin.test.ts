...
const mockEEBuildApis = { api: 'EEBuildApis' } as unknown as ApiFactory<
  ...
  any
>;
const mockGitRepositoriesExtensionsApiFactory = {
  api: 'gitRepositoriesExtensions',
...
jest.mock('@ansible/backstage-rhaap-common/gitRepositoriesExtensions', () => ({
  defaultGitRepositoriesExtensionsApiFactory:
    mockGitRepositoriesExtensionsApiFactory,
}));
jest.mock('./routes', () => ({
...
describe('self-service plugin module', () => {
  ...
  it('calls createPlugin with expected id and apis', () => {
    ...
    expect(callArg.apis).toContain(mockGitRepositoriesExtensionsApiFactory);
