export const isValidEntityName = (
  ...
  if (trimmedName.length < 1) {
    return {
      valid: false,
      error: `${field} must be at least 1 character long`,
    };
  }
  ...
  if (trimmedName.length > 63) {
    return {
      valid: false,
      error: `${field} must be at most 63 characters long`,
    };
  }
  ...
  if (!validPattern.test(trimmedName)) {
    return {
      valid: false,
      error: `${field} must consist of alphanumeric characters [a-z0-9A-Z] separated by hyphens, underscores, or dots`,
    };
  }
  ...
  if (
    ...
    return {
      valid: false,
      error: `${field} should not end with .yaml or .yml. A .yaml extension will automatically be added to the generated EE definition file name.`,
    };
  }
