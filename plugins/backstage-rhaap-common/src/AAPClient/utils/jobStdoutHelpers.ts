...
export function redactSensitiveLogMessage(message: string): string {
  ...
  for (const key of SENSITIVE_KEYS) {
    const escapedKey = escapeRegExp(key);
    // Assignments: password=secret, token: abc123, or password = "first second".
    // Match complete quoted values (incl. escapes) before the unquoted alternative
    // so whitespace inside quotes is not truncated mid-secret.
    // (authorization / bearer use BASIC_AUTH_PATTERN / BEARER_PATTERN)
    redacted = redacted.replace(
      new RegExp(
        String.raw`(\b${escapedKey})(\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;]+)`,
        'gi',
      ),
      `$1$2${REDACTION_PLACEHOLDER}`,
    );
