...
export class GitlabCrawler extends BaseScmCrawler {
  async discoverGalaxyFiles(
    ...
    this.logger.info(
      `Starting galaxy.yml discovery in ${repos.length} projects`,
    );
  ...
  async discoverGalaxyFilesInRepos(
    ...
    if (skippedRepos.length > 0) {
      this.logger.info(
        `Skipped ${skippedRepos.length} projects with no collections:`,
      );
    ...
    this.logger.info(
      `Discovered ${discovered.length} galaxy.yml files in ${repos.length} projects`,
    );
    return discovered;
  }
