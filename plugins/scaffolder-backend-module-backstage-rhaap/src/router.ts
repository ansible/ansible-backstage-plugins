...
export async function createRouter(options: {
  ...
  router.get('/get_ee_definition', async (req, res) => {
    const required = ['scm', 'owner', 'repository', 'subdir', 'filename'];
    const missing = required.filter(p => !req.query[p]);
    if (missing.length > 0) {
      return res.status(400).json({
        error: `Missing required query parameters: ${missing.join(', ')}\n`,
    ...
    if (!allowedScm.includes(scm)) {
      return res.status(400).json({
        error: `Unsupported SCM type '${scm}'. Supported values: ${allowedScm.join(', ')}`,
      });
    }
    ...
    if (!repoExists) {
      return res
        .status(404)
        .send(
          'Unable to fetch EE definition because the repository does not exist\n',
        );
    }

    let definitionContent: string = '';
    if (scm === 'Github') {
      definitionContent = await useCaseMaker.fetchGithubFileContent({
        owner: owner,
        repo: repository,
      ...
      definitionContent = await useCaseMaker.fetchGitlabFileContent({
        owner: owner,
        repo: repository,
        filePath: `${subdir}/${filename}`,
        branch: 'main',
    ...
    res.type('text/yaml');
    return res.send(definitionContent);
  });
