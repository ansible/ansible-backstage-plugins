...
describe('createRouter', () => {
  ...
  describe('GET /get_ee_definition', () => {
    it('should return 400 when filename parameter is missing', async () => {
      const response = await request(app).get('/get_ee_definition').query({
        scm: 'Github',
        owner: 'test-owner',
        repository: 'test-repo',
        subdir: 'ee',
    ...
    it('should return 404 when repository does not exist', async () => {
      ...
      expect(response.status).toBe(404);
      expect(response.text).toContain('repository does not exist');
      expect(
        mockUseCaseMakerInstance.fetchGithubFileContent,
      ).not.toHaveBeenCalled();
    });

    it('should successfully fetch EE definition for GitHub repository', async () => {
      mockUseCaseMakerInstance.checkIfRepositoryExists.mockResolvedValue(true);
      const mockDefinitionContent =
        'version: 3\nimages:\n  base_image:\n    name: quay.io/ee-minimal:latest';
      mockUseCaseMakerInstance.fetchGithubFileContent.mockResolvedValue(
        mockDefinitionContent,
      );
      ...
      expect(response.status).toBe(200);
      expect(response.text).toBe(mockDefinitionContent);
    ...
    it('should successfully fetch EE definition for GitLab repository', async () => {
      mockUseCaseMakerInstance.checkIfRepositoryExists.mockResolvedValue(true);
      const mockDefinitionContent =
        'version: 3\nimages:\n  base_image:\n    name: quay.io/ee-minimal:latest';
      mockUseCaseMakerInstance.fetchGitlabFileContent.mockResolvedValue(
      ...
      const response = await request(app).get('/get_ee_definition').query({
        ...
        repository: 'test-repo',
        subdir: 'ee',
        filename: 'execution-environment.yaml',
      });
      ...
      expect(
        ...
        owner: 'test-owner',
        repo: 'test-repo',
        filePath: 'ee/execution-environment.yaml',
        branch: 'main',
      });
    });
  });
