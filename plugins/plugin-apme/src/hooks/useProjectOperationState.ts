...
const TERMINAL_STATUSES = new Set<ProjectOperationStatus>([
  'completed', 'pr_submitted', 'failed', 'expired', 'cancelled',
]);
...
export function useProjectOperationState(projectId: string) {
  ...
  const connect = useCallback(async () => {
    ...
    if (!baseUrlRef.current) {
      try { baseUrlRef.current = await api.getProxyBaseUrl(); } catch { return; }
    }
    const base = baseUrlRef.current;
    const es = new EventSource(`${base}/projects/${projectId}/operation/events`);
    esRef.current = es;

    es.addEventListener('snapshot', (e: MessageEvent) => {
      if (!mountedRef.current) return;
      try { setState(JSON.parse(e.data) as ProjectOperationState); setConnected(true); } catch { /* ignore */ }
    });
    es.addEventListener('status_changed', (e: MessageEvent) => {
      if (!mountedRef.current) return;
      try {
        const d = JSON.parse(e.data) as { status: string; error?: string };
        setState(prev => prev ? { ...prev, status: d.status as ProjectOperationStatus, ...(d.error ? { error: d.error } : {}) } : prev);
      } catch { /* ignore */ }
    });
    es.addEventListener('progress', (e: MessageEvent) => {
      ...
      try {
        ...
        setState(prev => prev ? { ...prev, progress: [...prev.progress, entry] } : prev);
      } catch { /* ignore */ }
    });
    ...
    es.addEventListener('approval_ack', (e: MessageEvent) => {
      ...
      try {
        ...
        setState(prev => prev ? { ...prev, status: 'applying' as ProjectOperationStatus, result: prev.result ? { ...prev.result, ai_accepted: d.applied_count } : prev.result } : prev);
      } catch { /* ignore */ }
    });
    ...
    es.onerror = () => {
      if (!mountedRef.current) return;
      es.close(); esRef.current = null; setConnected(false);
  ...
  const poll = useCallback(async () => {
    try {
      const s = await api.getProjectOperation(projectId) as ProjectOperationState | null;
      if (!mountedRef.current) return;
      if (!s) { setState(null); return; }
      setState(s);
      if (!TERMINAL_STATUSES.has(s.status)) connect();
    } catch { if (mountedRef.current) setState(null); }
  ...
  const clear = useCallback(() => { cleanup(); setState(null); }, [cleanup]);
