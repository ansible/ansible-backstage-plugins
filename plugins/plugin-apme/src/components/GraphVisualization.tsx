...
const NODE_COLORS: Record<string, string> = {
  playbook: '#f85149', play: '#f0883e', role: '#d2a8ff', taskfile: '#79c0ff',
  task: '#58a6ff', handler: '#3fb950', block: '#d29922', vars_file: '#8b949e',
  module: '#56d364', collection: '#a371f7',
};

const MARKER_COLORS: Record<string, string> = {
  flow: '#8b949e', contains: '#30363d', import: '#58a6ff', include: '#d2a8ff',
  dependency: '#f0883e', data_flow: '#f778ba', notify: '#3fb950', listen: '#3fb950',
  vars_include: '#79c0ff', rescue: '#f85149', always: '#d29922', invokes: '#56d364',
  py_imports: '#a371f7',
};

const GROUP_COLORS: Record<string, string> = { play: '#f0883e', role: '#d2a8ff', block: '#d29922' };

interface NodeInfo {
  id: string; type: string; name: string; fullName: string;
  module: string; modLabel: string; file: string; line: number;
  scope: string; yaml: string; w: number; h: number;
}
...
export const GraphVisualization = ({ data }: { data: GraphData }) => {
  ...
  const [groupState, setGroupState] = useState<Record<string, boolean>>({ play: false, role: false, block: false });
  ...
  const buildGraph = useCallback(async () => {
    ...
    g.setGraph({ rankdir: 'TB', nodesep: 20, ranksep: 40, edgesep: 6, marginx: 40, marginy: 40 });
    ...
    data.nodes.forEach(n => {
      ...
      const h = mod ? 38 : 26;
      nodeMap[n.id] = { id: n.id, type: nt, name: label, fullName: rawName, module: mod, modLabel, file: (dd.file_path as string) || '', line: (dd.line_start as number) || 0, scope: (dd.scope as string) || 'owned', yaml: (dd.yaml_lines as string) || '', w, h };
      g.setNode(n.id, { width: w, height: h });
    });
    nodeMapRef.current = nodeMap;

    const execEdges = (data.execution_edges || []).filter(e => nodeSet.has(e.source) && nodeSet.has(e.target));
    execEdges.forEach((e, i) => g.setEdge(e.source, e.target, { minlen: 1 }, 'exec_' + i));
    dagre.layout(g);

    const container = svg.append('g');
    const zoomBehavior = d3.zoom<SVGSVGElement, unknown>().scaleExtent([0.02, 4]).on('zoom', (ev: any) => container.attr('transform', ev.transform));
    svg.call(zoomBehavior);
    zoomBehaviorRef.current = zoomBehavior;

    const defs = svg.append('defs');
    Object.entries(MARKER_COLORS).forEach(([type, color]) => {
      defs.append('marker').attr('id', 'arr-' + type).attr('viewBox', '0 -4 8 8').attr('refX', 8).attr('refY', 0).attr('markerWidth', 5).attr('markerHeight', 5).attr('orient', 'auto').append('path').attr('d', 'M0,-3L8,0L0,3').attr('fill', color);
    });
    ...
    function edgePoint(nodeId: string, toX: number, toY: number) {
      const dn = g.node(nodeId); const nm = nodeMap[nodeId];
    ...
    function drawEdge(group: any, srcId: string, tgtId: string, cls: string) {
      const sn = g.node(srcId); const tn = g.node(tgtId);
      ...
      group.append('path').attr('class', 'graph-edge ' + cls).attr('d', `M${p1.x},${p1.y} C${p1.x + p1.nx * cp},${p1.y + p1.ny * cp} ${p2.x + p2.nx * cp},${p2.y + p2.ny * cp} ${p2.x},${p2.y}`).attr('marker-end', 'url(#arr-' + cls.split(' ')[0] + ')');
    }
    ...
    edgeData.forEach(e => { if (e.type !== 'contains' && e.type !== 'include' && e.type !== 'import') drawEdge(xEdgeGroup, e.source, e.target, e.type); });

    const tooltip = d3.select(containerRef.current).select('.graph-tooltip');
    const nodeGroup = container.append('g');
    Object.values(nodeMap).forEach(n => {
      const dn = g.node(n.id); if (!dn) return;
      const x = dn.x - n.w / 2, y = dn.y - n.h / 2;
      const color = NODE_COLORS[n.type] || '#484f58';
      const grp = nodeGroup.append('g').attr('transform', `translate(${x},${y})`);
      grp.append('rect').attr('class', 'graph-node ' + n.scope).attr('width', n.w).attr('height', n.h).attr('fill', color).attr('stroke', color);
      if (n.module) {
        grp.append('text').attr('class', 'graph-node-label').attr('x', 8).attr('y', 12).text(n.name);
        grp.append('text').attr('class', 'graph-node-badge').attr('x', 8).attr('y', 28).attr('fill', color).text(n.modLabel);
      } else {
        grp.append('text').attr('class', 'graph-node-label').attr('x', 8).attr('y', n.h / 2).text(n.name);
      }
      grp.on('mouseover', () => {
        let h = `<span class="f">type:</span> <span class="v">${escapeHtml(n.type)}</span>`;
        if (n.fullName) h += ` &middot; <span class="v">${escapeHtml(n.fullName)}</span>`;
        h += '<br>';
        if (n.module) h += `<span class="f">module:</span> <span class="v mod">${escapeHtml(n.module)}</span><br>`;
  ...
  const fitAll = useCallback(async () => {
    if (!svgRef.current || !containerRef.current || !graphRef.current || !zoomBehaviorRef.current) return;
    const d3 = await import('d3');
    const svg = d3.select(svgRef.current);
    ...
    const scale = Math.min(W / (gw + 80), H / (gh + 80), 1.5) * 0.9;
    const tx = (W - gw * scale) / 2, ty = (H - gh * scale) / 2;
    svg.transition().duration(500).call(zoomBehaviorRef.current.transform, d3.zoomIdentity.translate(tx, ty).scale(scale));
  }, []);
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '70vh', minHeight: 400, background: '#0d1117', borderRadius: 8, overflow: 'hidden' }}>
      <style>{`
      ...
      <div style={{ position: 'absolute', top: 12, left: 12, padding: '8px 12px', fontSize: 12, display: 'flex', gap: 8, flexWrap: 'wrap', background: '#161b22', border: '1px solid #30363d', borderRadius: 6, zIndex: 10 }}>
        <button type="button" onClick={fitAll} style={{ background: '#21262d', border: '1px solid #30363d', color: '#c9d1d9', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: 11 }}>Fit</button>
        <button type="button" onClick={zoomIn} style={{ background: '#21262d', border: '1px solid #30363d', color: '#c9d1d9', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: 11 }}>+</button>
        <button type="button" onClick={zoomOut} style={{ background: '#21262d', border: '1px solid #30363d', color: '#c9d1d9', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: 11 }}>&minus;</button>
      </div>
      <div style={{ position: 'absolute', top: 12, right: 12, padding: '12px 16px', fontSize: 11, background: '#161b22', border: '1px solid #30363d', borderRadius: 6, zIndex: 10, color: '#c9d1d9' }}>
        <div style={{ fontWeight: 600, marginBottom: 6 }}>Nodes</div>
        {Object.entries(NODE_COLORS).slice(0, 8).map(([type, color]) => (
          <div key={type} style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '3px 0' }}>
            <div style={{ width: 12, height: 12, borderRadius: 2, background: color, flexShrink: 0 }} />
            {type}
          </div>
        ))}
      </div>
      <div style={{ position: 'absolute', bottom: 12, left: 12, padding: '8px 14px', fontSize: 12, background: '#161b22', border: '1px solid #30363d', borderRadius: 6, zIndex: 10, color: '#c9d1d9' }}>
