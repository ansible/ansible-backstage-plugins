 ...
 * Copyright Red Hat
 *
 * ADR-010: APME registers Git Repos extension surfaces via gitRepositoriesExtensionsApiRef.
 * Exported for dynamic-plugin apiFactories wiring (portal / rhdh-local).
 */
...
const LazyApmeFleetQualityTab = lazy(() =>
  import('../components/FleetQualityTab').then(m => ({
    default: m.FleetQualityTab,
  })),
);

const LazyApmeQualitySettingsTab = lazy(() =>
  import('../components/ApmeQualitySettingsTab').then(m => ({
    default: m.ApmeQualitySettingsTab,
  })),
);

const LazyEntityQualityTab = lazy(() =>
  import('../components/EntityQualityTab').then(m => ({
    default: m.EntityQualityTab,
  })),
);

const LazyApmeRepositoryOverviewCard = lazy(() =>
  import('../components/ApmeRepositoryOverviewCard/ApmeRepositoryOverviewCard').then(
    m => ({
      default: m.ApmeRepositoryOverviewCard,
    }),
  ),
);

const LazyDependenciesTab = lazy(() =>
  import('../components/DependenciesTab/DependenciesTab').then(m => ({
    default: m.DependenciesTab,
  })),
);

const LazyApmeRepositoryCollectionsTab = lazy(() =>
  import('../components/ApmeRepositoryCollectionsTab/ApmeRepositoryCollectionsTab').then(
    m => ({
      default: m.ApmeRepositoryCollectionsTab,
    }),
  ),
);

const LazyApmeRepoStatusChip = lazy(() =>
  import('../components/ApmeRepoStatusChip').then(m => ({
    default: m.ApmeRepoStatusChip,
  })),
);

const LazyApmeRepositoryHeaderActions = lazy(() =>
  import('../components/ApmeRepositoryHeaderActions/ApmeRepositoryHeaderActions').then(
    m => ({
      default: m.ApmeRepositoryHeaderActions,
    }),
  ),
);
...
export const gitRepositoriesExtensionsApiFactory: AnyApiFactory =
  createApiFactory({
    ...
    factory: ({ configApi }) => {
      const apmeEnabled = isApmeEnabled(configApi);
      return apmeEnabled
