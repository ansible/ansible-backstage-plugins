...
export function validatePlatform(
  ...
  if (!spec || spec.scanner_validates) {
    return {
      valid: true,
      matchedHosts: hosts.map(h => h.hostname),
      mismatchedHosts: [],
      factsAvailable: true,
    };
  }
  ...
  for (const host of hosts) {
    const hasFacts = !!(
      ...
      host.ansible_distribution_major_version ||
      host.device_type
    );
    ...
    if (
      spec.os_family?.length &&
      host.ansible_os_family &&
      !spec.os_family.includes(host.ansible_os_family)
    ) {
      reasons.push(
        `OS family '${host.ansible_os_family}' not in [${spec.os_family.join(
          ', ',
        )}]`,
      );
    }
    ...
    if (
      spec.os_version?.length &&
      host.ansible_distribution_major_version &&
      !spec.os_version.includes(host.ansible_distribution_major_version)
    ) {
      reasons.push(
        `OS version '${
          host.ansible_distribution_major_version
        }' not in [${spec.os_version.join(', ')}]`,
      );
    }

    if (
      spec.device_type?.length &&
      host.device_type &&
      !spec.device_type.includes(host.device_type)
    ) {
      reasons.push(
        `Device type '${host.device_type}' not in [${spec.device_type.join(
          ', ',
        )}]`,
      );
    }
