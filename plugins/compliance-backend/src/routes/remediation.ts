...
import { requirePermission, getUserAapToken, extractUserIdentity } from './permissions';
import { isNonEmptyString, isPositiveInteger, isBoolean, isArray } from './validation';
...
export function registerRemediationRoutes(
  ...
  router.post('/remediate', async (req, res) => {
    ...
    if (!resolvedScanId) {
      const authScan = await database.getAuthoritativeScan(body.profileId, body.inventoryId);
      if (!authScan) {
        res.status(400).json({
    ...
    try {
      ...
      let execution: Awaited<ReturnType<typeof database.createExecution>> | null = null;
      ...
      if (resolvedProfileId && !execution) {
        const running = await database.getRunningExecutionForInventory(body.inventoryId);
        logger.warn(`Concurrent execution guard: blocked remediation on inventory ${body.inventoryId}`);
        res.status(409).json({
          error: {
            name: 'ConflictError',
      ...
      const findings = await service.getFindings(undefined, remediateRequest.profileId, remediateRequest.inventoryId);
      logger.info(`Findings for remediation (profile=${remediateRequest.profileId}, inventory=${remediateRequest.inventoryId}): ${findings.length} rules`);
      ...
      if (plan.groups.length > 0 && service.getDataSource() === 'live') {
        try {
          ...
          const planHosts = new Set(plan.groups.flatMap(g => g.limit?.split(/[,\s]+/).map(h => h.trim()).filter(Boolean) ?? []));
          const missing = [...planHosts].filter(h => !inventoryHostSet.has(h));
          if (missing.length > 0) {
            logger.warn(
              `Host validation: ${missing.length} hosts from plan not found in inventory ${remediateRequest.inventoryId}: ${missing.join(', ')}`,
            );
            res.status(422).json({
              error: `${missing.length} host(s) from scan findings not found in target inventory: ${missing.slice(0, 5).join(', ')}${missing.length > 5 ? ` (+${missing.length - 5} more)` : ''}. The inventory may have changed since the last scan.`,
            });
            return;
          }
        } catch (hostErr) {
          logger.debug(`Host validation skipped: ${hostErr instanceof Error ? hostErr.message : hostErr}`);
        }
      }
      ...
      let result;
      try {
        result = await service.launchRemediation(remediateRequest, findings, userToken);
      } catch (launchErr) {
        if (execution) {
          await database.updateExecutionStatus(execution.id, {
            status: 'failed',
      ...
      const isUserError = msg.includes('No failing hosts') || msg.includes('No compliance remediate job template');
      if (isUserError) {
  ...
  router.get('/remediation-executions', async (req, res) => {
    ...
    if (profileId) {
      const executions = await database.getExecutionsByProfileId(profileId, limit);
      res.json(executions);
    } else {
  ...
  router.get('/chain/:executionId', async (req, res) => {
    ...
    if (execution.verificationScanId) scanIds.push(execution.verificationScanId);

    const [assessmentScan, verificationScan, batchStats] = await Promise.all([
      execution.informingScanId ? database.getScanById(execution.informingScanId) : null,
      execution.verificationScanId ? database.getScanById(execution.verificationScanId) : null,
      scanIds.length > 0 ? database.getBatchScanStatsAggregated(scanIds) : {},
    ]);

    const toStats = (scanId: string | null) => {
      if (!scanId) return null;
      const s = (batchStats as Record<string, { pass: number; fail: number; rules: number; hosts: number }>)[scanId];
      if (!s) return null;
      return { pass: s.pass, fail: s.fail, rules: s.rules, hosts: s.hosts };
    };

    let delta: { fixed: number; regressed: number; unchanged: number } | null = null;
  ...
  router.patch('/remediation-executions/:id', async (req, res) => {
    ...
    const { status, completedAt, elapsedSeconds, rulesApplied, rulesFailed,
            hostsTargeted, hostsSucceeded, hostsFailed, primaryJobId, allJobIds } = req.body;
    ...
    await database.updateExecutionStatus(req.params.id, {
      ...
      completedAt, elapsedSeconds, rulesApplied, rulesFailed,
      hostsTargeted, hostsSucceeded, hostsFailed, primaryJobId, allJobIds,
    });
    ...
    if (status && terminalStatuses.includes(status) && execution.primaryJobId) {
      const scanRecord = await database.getScanByWorkflowJobId(execution.primaryJobId);
      if (scanRecord && scanRecord.scanner === 'remediation') {
        const scanStatus = status === 'succeeded' ? 'completed' : 'failed';
        await database.updateScanStatus(scanRecord.id, scanStatus, completedAt || new Date().toISOString());
      }
    }
  ...
  router.get('/remediation-error-details', async (req, res) => {
    ...
    const jobIds = jobIdsParam.split(',').map(Number).filter(n => !Number.isNaN(n)).slice(0, 20);
    if (jobIds.length === 0) {
    ...
    const userToken = getUserAapToken(req);
    try {
      const details = await service.fetchRemediationErrorDetails(jobIds, userToken);
      res.json({ errorDetails: details });
    } catch (err) {
      logger.warn(`Failed to fetch remediation error details: ${err instanceof Error ? err.message : err}`);
      res.json({ errorDetails: null });
    }
  });
  ...
  router.patch('/remediation-profiles/:id', async (req, res) => {
    ...
    if (status === 'archived') {
      ...
      if (isPinned) {
        res.status(409).json({ error: 'Cannot archive a profile pinned as a golden standard. Unpin it first.' });
        return;
      }
    }
    const updated = await database.updateRemediationProfileStatus(req.params.id, status);
    if (!updated) {
  ...
  router.get('/remediation-profiles', async (req, res) => {
    ...
    try {
      ...
      for (const exec of running) {
        ...
        if (now - (state.executionReconcileThrottle.get(exec.id) ?? 0) < state.STALE_CHECK_INTERVAL_MS) continue;
        ...
        try {
          ...
          const resolvedStatus = job.status === 'successful' ? 'succeeded' : 'failed';
          logger.info(`Reconciled execution ${exec.id} (job ${exec.primaryJobId}) → ${resolvedStatus}`);
          ...
          logger.debug(`Reconciliation: Controller unreachable for execution ${exec.id}, skipping: ${controllerErr instanceof Error ? controllerErr.message : controllerErr}`);
        }
      }
    } catch (err) {
      logger.warn(`Execution reconciliation failed: ${err instanceof Error ? err.message : err}`);
    }
