...
export function registerInventoryRoutes(
  ...
  router.get('/inventory/:inventoryId/host-posture', async (req, res) => {
    ...
    if (!profileId) {
      res
        .status(400)
        .json({ error: 'profile or profileId query parameter is required' });
      return;
    }

    try {
      const baselineView = req.query.baselineView === 'true';
      const result = await service.getHostPosture(inventoryId, profileId, req, {
        baselineView,
      });
      res.json(result);
    } catch (error) {
      ...
      logger.error(
        `Failed to get host posture for inventory ${inventoryId}: ${msg}`,
      );
  ...
  router.get(
    '/inventory/:inventoryId/host/:hostname/findings',
    async (req, res) => {
      const inventoryId = Number(req.params.inventoryId);
      if (
        isNaN(inventoryId) ||
        ...
        res
          .status(400)
          .json({ error: 'inventoryId must be a positive integer' });
        return;
      }
      ...
      try {
        const result = await service.getHostFindings(
          inventoryId,
          hostname,
          profileId,
          limit,
        ...
        const msg = error instanceof Error ? error.message : String(error);
        logger.error(
          `Failed to get host findings for ${hostname} in inventory ${inventoryId}: ${msg}`,
        );
        res.status(500).json({ error: msg });
      }
    },
