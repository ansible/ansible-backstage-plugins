...
export type EnvOptions = {
  ...
  // ANSIBLE ENHANCEMENT: External system services for RBAC checks
    ...
    if (isPluginEnabled) {
      ...
      env.policy.setPolicy(
        await RBACPermissionPolicy.build(
          ...
          pluginPermMetaData,
          env.auth,
          env.aapService,       // ANSIBLE ENHANCEMENT: AAP service
          env.aapTokenLookup,   // ANSIBLE ENHANCEMENT: AAP token lookup
        ),
      );
    } else {
      env.logger.warn(
