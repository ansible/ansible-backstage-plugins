...
**AI attribution:** if you used an AI coding tool, add an `Assisted-by` or `Generated by` trailer (not `Co-authored-by`) — no email required:
...
The `commit-msg` hook rejects `Co-authored-by` lines that name AI tools or bots.
